...
class Board:
    ...
    def meta(self, n):
        ...
        if p.get("is_fork"):
            bits.append("fork")
        if p.get("updated_at"):
            iso = esc(p["updated_at"])
            fallback = esc(p["updated_at"].replace("T", " ").replace("Z", " UTC"))
            bits.append(f'updated <span class="ts" data-iso="{iso}">{fallback}</span>')
        if "files" in m:
